...
function ProjectCard({ project, onClick }: { project: Project; onClick: () => void }) {
  ...
  return (
    ...
    >
      {/* Image Section - Now using Next.js Image */}
      <div className="relative h-48 bg-gradient-to-br overflow-hidden">
        {project.image ? (
          ...
          // Fallback gradient with icon when no image
        ...
        {/* Hover overlay */}
        <div className="absolute inset-0 bg-dark-900/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center z-10">
...
// ProjectModal remains almost the same (you can also improve image here if you want)
    ...
    >
      ...
      >
        ...
        <div className="relative h-56 bg-gradient-to-br overflow-hidden">
          ...
          <button
            onClick={onClose}
            className="absolute top-4 right-4 w-8 h-8 rounded-full bg-dark-900/60 flex items-center justify-center hover:bg-dark-900 transition-colors text-white z-10"
          >
            ✕
          </button>
        </div>
